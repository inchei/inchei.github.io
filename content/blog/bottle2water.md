...
date = 2020-05-03
updated = 2020-05-03
aliases = ["blog/2020/05/03/bottle2water/"]
+++

在學校憋了三個星期，數學課上想到了這個經典問題。雖未習算法，深覺此為練習遞歸之機，就在草稿紙上摸了起來。上次幹這種事，還是初三上學期期末考的時候瞎編的干支紀年算法，只是當時我年輕地忘記了干支紀年是跟著農曆來的。回家謄寫並修改一番，或許這個問題太簡單了，然而對於無門系統學習的我來說，想出題目並自己解決還是很有意思的。

空瓶換水的問題，我想到了兩種。一種是**已知所需瓶數求最少買幾瓶**，一種是**已知手頭空瓶求最多換幾瓶**。用遞歸方法解決，首先思考遞歸函數的輸入變量。在第一種問題中，不斷改變所需水，第二種則是改變手頭空瓶。這樣一來，解決這兩個問題實在簡單。

不過，有一點我是靠著做過這個小學數學題才能想起來的，即當手頭空瓶數差一瓶為兑換所需瓶數時，可以借一瓶喝掉再換一瓶還回去。
...
def bottle2waterA(target, key=3, bought=0, exchanged=0):
    if target > 0:
        ...
        return bought
```
不同於第二個問題，這裡的 `exchanged` 和 `bought` 都被我拿來當條件了，不清楚如何再簡化。
...
```
這個簡化同時也說明了對於遞歸算法，**應當優先考慮不借助中間變量**。下次再寫遞歸，第一句上 `return` 好了。
